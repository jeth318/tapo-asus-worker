...
class RouterInfo:
    ...
    def __authenticate(self, ipaddress, username, password):
        ...
        try:
            r = requests.post(url='http://{}/login.cgi'.format(ipaddress),
    ...
    def get_traffic_total(self):
        ...
        tx = int(meas_2['netdev']['INTERNET_tx'],
                 base=16) * 8 / 1024 / 1024 / 2
        rx = int(meas_2['netdev']['INTERNET_rx'],
                 base=16) * 8 / 1024 / 1024 / 2
    ...
    def get_traffic(self):
        ...
        tx = int(meas_2['netdev']['INTERNET_tx'],
                 base=16) * 8 / 1024 / 1024 / 2
        totaldata['sent'] = tx
        tx -= int(meas_1['netdev']['INTERNET_tx'],
                  base=16) * 8 / 1024 / 1024 / 2
        persec['tx'] = tx
        rx = int(meas_2['netdev']['INTERNET_rx'],
                 base=16) * 8 / 1024 / 1024 / 2
        totaldata['recv'] = rx
        rx -= int(meas_1['netdev']['INTERNET_rx'],
                  base=16) * 8 / 1024 / 1024 / 2
        persec['rx'] = rx
    ...
    def findConnectedDevice(self, macTrigger):
        macList = json.loads(self.get_online_clients())
        for device in macList:
            if macTrigger in device["mac"]:
                return True
        return False
